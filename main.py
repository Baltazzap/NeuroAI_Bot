                ...
                if member:
                    ...
                    await send_log(
                        ...
                        [
                            ...
                            {"name": "Длительность", "value": f"`{mute_data['duration']}`", "inline": True}
                        ]
                    )
        except Exception as e:
...
async def mute_user(member, reason, channel=None):
    try:
        ...
        try:
            await member.send(f"Вы были заглушены автоматически.\n\nПричина: {reason}\n\nОбратитесь в тикет для разблокировки.")
        except:
            pass
    except Exception as e:
...
async def on_member_join(member):
    ...
    if account_age < AUTO_MOD_CONFIG["new_account_threshold"]:
        try:
            ...
            print(f"Ошибка при муте нового аккаунта: {e}")
    ...
    try:
        ...
        if auto_role:
            ...
            print(f"Роль {AUTO_ROLE_ID} не найдена на сервере")
    except Exception as e:
    ...
    try:
        ...
        if channel:
            ...
            print(f"Канал приветствий {WELCOME_CHANNEL_ID} не найден")
    except Exception as e:
    ...
    try:
        ...
        print(f"Ошибка отправки лога входа: {e}")
...
async def on_ready():
    ...
    async def slash_mute(interaction: discord.Interaction, member: discord.Member, duration: int, reason: str = "Нарушение правил"):
        ...
        await send_log(
            ...
            [
                ...
                {"name": "Причина", "value": f"`{reason}`", "inline": True},
                {"name": "Длительность", "value": f"`{duration} мин.`", "inline": True}
            ]
        )
    ...
    async def slash_unmute(interaction: discord.Interaction, member: discord.Member):
        ...
        if mute_role in member.roles:
            ...
            await interaction.response.send_message("У пользователя нет роли мута.", ephemeral=True)
    ...
    async def slash_warn(interaction: discord.Interaction, member: discord.Member, reason: str = "Нарушение правил"):
        ...
        await send_log(
            ...
            [
                ...
                {"name": "Предупреждений", "value": f"`{warns[member.id]}`", "inline": True}
            ]
        )
        await interaction.response.send_message(f"{member.mention} предупреждён. Причина: {reason}\nВсего предупреждений: {warns[member.id]}/3")
    ...
    print(f"Авто-модерация: активна")
    ...
    await send_log(bot, "Система запущена", "**AI кардинал** подключился.", 0x2ECC71, [
        ...
        {"name": "Авто-мод", "value": "`Активен`", "inline": True}
    ])
...
async def prefix_unmute(ctx, member: discord.Member):
    ...
    if mute_role in member.roles:
        ...
        await ctx.send("У пользователя нет роли мута.", delete_after=5)
...
async def prefix_warn(ctx, member: discord.Member, *, reason: str = "Нарушение правил"):
    ...
    await ctx.send(f"{member.mention} предупреждён. Причина: {reason}\nВсего предупреждений: {warns[member.id]}/3", delete_after=10)
...
async def prefix_clear(ctx, amount: int):
    ...
    if amount < 1 or amount > 100:
        await ctx.send("Количество должно быть от 1 до 100.", delete_after=5)
        return
    
    deleted = await ctx.channel.purge(limit=amount)
    await ctx.send(f"Удалено {len(deleted)} сообщений.", delete_after=10)
